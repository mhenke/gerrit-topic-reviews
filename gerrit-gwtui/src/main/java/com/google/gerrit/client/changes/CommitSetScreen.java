...
public class CommitSetScreen extends ChangeSetListScreen implements
    ChangeListScreen {
  public static CommitSetScreen forQuery(String query) {
    return forQuery(query, PageLinks.TOP);
  }

  public static CommitSetScreen forQuery(String query, String position) {
    return new CommitSetScreen(KeyUtil.encode(query), position);
  }

  private final String query;

  public CommitSetScreen(final String encQuery, final String positionToken) {
    super("changeset," + encQuery, positionToken);
  ...
  protected void onInitUI() {
    super.onInitUI();
    // TODO
    // Those are the titles we want to show ?
  ...
  public void getDetails() {
    // TODO
    // This is temporal, we are fetching the detalis
    // from a single change only as a PoC
    Util.DETAIL_SVC.changeDetail(changes.get(0).getId(), saveDetails());
  }
